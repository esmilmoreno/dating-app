...
export default function Dislikes({ navigation }) {
  ...
  const { myProfile, likeProfile } = useAuth()
  ...
  const refresh = async () => {
    ...
    try {
      const filterArray = myProfile.dislikes.length > 0 ? myProfile.dislikes : ["placeholder"]
      const q = query(collection(db, "profiles"), where("uid", "in", filterArray))
  ...
  return (
    <View style={{flex: 1}}>
      ...
      {selectedProfileIds.length > 0 && (
        <TouchableOpacity onPress={removeDislikes} style={[styles.action, { backgroundColor: colors.primary }]}>
          <Text style={styles.actionText}>Like {selectedProfileIds.length} profiles</Text>
        </TouchableOpacity>
      )}
    </View>
  )
